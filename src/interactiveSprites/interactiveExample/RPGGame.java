...
//		}
		...
		if(keyPressed(KeyEvent.VK_A) && playerTurn == HERO){
			if(gameState == BATTLE) {
				...
				if(heroEnergy>=1) {
					...
					bM = "BIG CAT USES 'COMPLIMENT'! GOOD FOR " + Integer.toString(temp-girlEnergy) + " DAMAGE!" ;
		...
		if(keyPressed(KeyEvent.VK_S)){
			if(gameState == BATTLE ) {
				...
				if(heroEnergy>=1) {
					int temp = girlEnergy;
					girlEnergy = girlEnergy - 10 -generator.nextInt(25);
					bM = "BIG CAT USES 'FUNNY STORY'! GOOD FOR " + Integer.toString(temp-girlEnergy) + " DAMAGE!" ;
					bM2 = "PRESS [SPACE] TO CONTINUE";
					playerTurn = GIRL;
				} else {
					bM = "GIRL DEFEATS AND EMBARRASSES BIG CAT!";
		...
		if(keyPressed(KeyEvent.VK_D ) && playerTurn == HERO){
			if(gameState == BATTLE) {
				...
				if(heroEnergy>=1) {
					int temp = girlEnergy;
					girlEnergy = girlEnergy - 10 -generator.nextInt(25);
					bM = "BIG CAT USES 'FINANCE LINGO'! GOOD FOR " + Integer.toString(temp-girlEnergy) + " DAMAGE!" ;
					bM2 = "PRESS [SPACE] TO CONTINUE";
					playerTurn = GIRL;
				} else {
					bM = "GIRL DEFEATS AND EMBARRASSES BIG CAT!";
		...
		if(keyPressed(KeyEvent.VK_F ) && playerTurn == HERO){
			if(gameState == BATTLE && phoneNumbers>2) {
				...
				if(heroEnergy>=1) {
					...
					bM = "BIG CAT USES 'TIPPY TAP'! INCREDIBLE! " + Integer.toString(temp-girlEnergy) + " DAMAGE!" ;
					bM2 = "PRESS [SPACE] TO CONTINUE";
					playerTurn = GIRL;
				} else {
					bM = "GIRL DEFEATS AND EMBARRASSES BIG CAT!";
		...
		if(keyPressed(KeyEvent.VK_SPACE) && playerTurn == GIRL){
			if(gameState == BATTLE) {
				...
				if (girlEnergy >= 1){
					...
					bM = "GIRL USES " + girlMoves[generator.nextInt(girlMoves.length)] +"! GOOD FOR "
	...
	public void render(Graphics2D g){
		...
		s4.render(g);
		spring.render(g);
		font.drawString(g, "PHONE NUMBERS: " + Integer.toString(phoneNumbers), 20, 15);
		...
		if (gameState==PLAYING) playMusic("AzaleaTown.mid");
		
		if(gameState==TALKING) {
			
			if (hero.getY()<=230) y = 320; else y = 0;
		...
		if(gameState == BATTLE) {
			...
			font.drawString(g, "BIG CAT ENERGY: " + Integer.toString(heroEnergy), 25, 175);
			font.drawString(g, "GIRL ENERGY: " + Integer.toString(girlEnergy), 400, 175);
			font.drawString(g, "A: COMPLIMENT", 25, 220);
			font.drawString(g, "S: FUNNY STORY", 25, 245);
			font.drawString(g, "D: FINANCE LINGO", 25, 270);
			if(phoneNumbers>2) font.drawString(g, "F: TIPPY TAP", 25, 295);
